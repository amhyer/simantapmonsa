...
@section('title', 'Push Kehadiran ke Dapodik - SIMANTAP')

@section('sidebar')
    @include('admin.partials.sidebar')
@endsection

@section('page_title', 'Push Kehadiran ke Dapodik')
@section('page_subtitle', 'Kirim data kehadiran siswa ke server Dapodik pusat')

@section('content')
    <div class="card" style="margin-bottom:24px">
        <div class="card-header">
            <h3><x-lucide-calendar-check class="w-5 h-5 mr-2" /> Push Kehadiran ke Dapodik</h3>
        </div>
        <div class="card-body">
            <div class="alert alert-info" style="margin-bottom:16px">
                <x-lucide-info class="w-5 h-5 mr-2" />
                <strong>Informasi:</strong> Push ini akan mengirim data kehadiran siswa (Hadir, Sakit, Izin, Alpa) ke server Dapodik pusat. Pastikan semester sudah dipilih dengan benar.
            </div>

            <form action="{{ route('admin.dapodik.push.kehadiran') }}" method="POST">
                @csrf
                <div class="card">
                    <div class="card-header">
                        ...
                        <div style="display:flex;gap:12px;align-items:end;flex-wrap:wrap;margin-bottom:16px">
                            ...
                            <div style="flex:0 0 auto">
                                <button type="submit" class="btn" style="height:40px">
                                    <x-lucide-upload class="w-4 h-4 mr-2" /> Push Kehadiran
                                </button>
                            </div>
                        </div>
                    </div>
